const input = `tjt{s<421:R,m<858:R,R}
pn{m<2884:vzk,kv}
...
const inputTest = `px{a<2006:qkq,m>2090:A,rfg}
pv{a>1716:R,A}
lnx{m>1548:A,A}
...
const workflows = workflowsString.split('\n').map((line) => {
	const bIndex = line.indexOf('{');
	const name = line.substring(0, bIndex);
	const conditions = line
		.substring(bIndex + 1, line.length - 1)
		.split(',')
		.map((conditionString) => {
			if (conditionString.indexOf(':') > 0) {
				const [cond, destination] = conditionString.split(':');

				const isBigger = cond.indexOf('>') > 0;
				const [param, value] = cond.split(isBigger ? '>' : '<');

				return { param, value: Number(value), isBigger, destination };
			}

			return { isDestination: true, destination: conditionString };
		});
...
parts.forEach((part) => {
	let pos = 'in';
	while (pos !== 'A' && pos !== 'R') {
		const wf = workflows.find(({ name }) => name === pos);

		wf.conditions.some((cond) => {
			if (cond.isDestination) {
				pos = cond.destination;
				return true;
			}

			if ((cond.isBigger && part[cond.param] > cond.value) || (!cond.isBigger && part[cond.param] < cond.value)) {
				pos = cond.destination;
				return true;
			}
	...
	if (pos === 'A') {
		total = total + Number(part.x) + Number(part.m) + Number(part.a) + Number(part.s);
	}
});
